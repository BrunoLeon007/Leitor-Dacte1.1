...
xmlFileInput.addEventListener('change', function(e) {
    ...
    reader.onload = function(evt) {
        try {
            ...
            if (matchCte && matchCte[1]) chaveCte = matchCte[1]; 
            ...
            while ((blocoMatch = regexBlocoNfe.exec(xmlText)) !== null) {
                // CORREÇÃO CRÍTICA: Pega o conteúdo de dentro do grupo 1 da tag infNFe
                const conteudoBloco = blocoMatch[1]; 
                
                // Busca chaves de 44 dígitos ou nós chNFe dentro desse bloco específico
                const regexChave44 = /[0-9]{44}/g;
                let chaveMatch = conteudoBloco.match(regexChave44);
                
                if (chaveMatch) {
                    chaveMatch.forEach(chaveCompleta => {
                        // Extrai as posições oficiais 26 a 34 da SEFAZ (9 dígitos)
                        const fragmentoNota = chaveCompleta.substring(25, 34);
    ...
    reader.readAsText(files[0]); // CORREÇÃO CRÍTICA: Lendo explicitamente o primeiro arquivo [0]
});
...
chaveBipadaInput.addEventListener('input', function() {
    ...
    if (chaveApenasNumeros.length === 44) {
        ...
        // Extrai exatamente o mesmo bloco de 9 dígitos mantendo os zeros (ex: "000393381")
        const fragmentoBipe = chaveApenasNumeros.substring(25, 34);
        const numeroNotaMapeado = fragmentoBipe.trim();

           

        // Verifica se já foi conferida anteriormente
        if (ultimasNotasConferidas.includes(numeroNotaMapeado)) {
        ...
        // Verifica correspondência exata de Strings de 9 dígitos
        const notaExisteNoXml = notasExtraidasDoXml.includes(numeroNotaMapeado);

        if (notaExisteNoXml) {
            ultimasNotasConferidas.push(numeroNotaMapeado);
...
function renderizarNotas() {
    ...
    if (notasExtraidasDoXml.length > 0) {
        notasExtraidasDoXml.forEach(nota => {
            ...
            // ID gerado estritamente com os 9 dígitos (ex: badge-xml-000393381)
            span.id = "badge-xml-" + nota;
...
function atualizarStatusConferencia() {
    notasExtraidasDoXml.forEach(notaXml => {
        const foiConferida = ultimasNotasConferidas.includes(notaXml);

        // Busca o ID exato mantendo a formatação original do XML
        const elementoBadge = document.getElementById("badge-xml-" + notaXml);
        if (elementoBadge) {
            if (foiConferida) {
